...
abstract class Magegiant_Fullpagecache_Helper_Cache_Blockhtml_Abstract extends Mage_Core_Helper_Abstract
{
    /**
    ...
    public function buildFullBaseCacheKey($block)
    {
        ...
        get_class($block) . '_' .
        $block->getTemplate() . '_' .
        Magegiant_Fullpagecache_Helper_Data::getDeviceKey();
    }
    ...
    {
        return array(
            Magegiant_Fullpagecache_Helper_Data::BLOCK_HTML,
    ...
    public function handleBlockhtmlCache($block, $cacheLifetime)
    {
        ...
        if ($this->isBlockCachable($block)) {
            $block->addData(array(
                ...
                'cache_key' => Magegiant_Fullpagecache_Helper_Data::formatCacheKey($this->buildCacheKey($block))
            ));
        }
    }
}