...
class Magegiant_Fullpagecache_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    const FULLPAGECACHE_REFRESH_CACHE_ON_PAGE = 'fullpagecache_refresh_cache_on_page';
    ...
    public static function getSimpleConfig()
    {
        if (!Mage::registry('simple_config')) {
            Varien_Profiler::start('fullpagecache_load_simple_config');
            Mage::setRoot();
            $config = new Mage_Core_Model_Config();
            $config->loadFile($config->getOptions()->getEtcDir() . DS . 'local.xml');
            Mage::register('simple_config', $config);
            Varien_Profiler::stop('fullpagecache_load_simple_config');
        }
    ...
    {
        $result = array(self::FULLPAGECACHE_REFRESH_CACHE_ON_PAGE);
        $simpleConfig = self::getSimpleConfig();

        if ($simpleConfig->getNode('global/fullpagecache/cache/query_string_filters')) {
            $queryStringFilters = $simpleConfig->getNode('global/fullpagecache/cache/query_string_filters')->asArray();
    ...
    public static function getCacheContainer()
    {
        if (!self::$_cacheContainer) {
            if (version_compare(Mage::getVersion(), '1.4.0', '>=')) {
                self::$_cacheContainer = new Magegiant_Fullpagecache_Model_Core_Cache_Container_V14();
            } else {
                self::$_cacheContainer = new Magegiant_Fullpagecache_Model_Core_Cache_Container_V13();
            }
        }
    ...
    public static function getDeviceKey()
    {
        if (is_null(Mage::registry('device_key'))) {
            ...
            if ($simpleConfig->getNode('global/fullpagecache/device_detector/class')) {
                $deviceDetectorClass = $simpleConfig->getNode('global/fullpagecache/device_detector/class')->asArray();

                if (!empty($deviceDetectorClass)) {
                    $deviceDetector = new $deviceDetectorClass();

                    if ($deviceDetector instanceof Magegiant_Fullpagecache_Helper_Device_IDetector) {
    ...
    {
        if (!Mage::getStoreConfig('fullpagecache/optimization_cdn/enabled')) {
            return $baseUrl;
        }
        ...
        $cn = Mage::getStoreConfig('fullpagecache/optimization_cdn/number');
