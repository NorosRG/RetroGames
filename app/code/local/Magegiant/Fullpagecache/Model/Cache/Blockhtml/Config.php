...
class Magegiant_Fullpagecache_Model_Cache_Blockhtml_Config extends Magegiant_Fullpagecache_Model_Cache_Config
{
    public function __construct()
    {
        $this->_init('fullpagecache/cache_blockhtml_config', 'fullpagecache/cache_blockhtml_config_collection');
    }
    ...
    public function tryBlockMatchWithCacheBlockhtmlConfig($block)
    {
        ...
        foreach ($collection as $cacheBlockhtmlConfig) {
            if ($cacheBlockhtmlConfig["activated"] == 1
                ...
                && (in_array(Mage::app()->getStore()->getId(), $cacheBlockhtmlConfig["store_id"]) || in_array(0, $cacheBlockhtmlConfig["store_id"]))
            ) {
    ...
    {
        $newCacheBlockhtmlConfig = Mage::getModel('fullpagecache/cache_blockhtml_config');
