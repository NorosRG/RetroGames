...
class Magegiant_Fullpagecache_Model_Cache_Blockhtml_Observer extends Varien_Object
{
    protected function _construct()
    {
        $this->setLearningModeEnabled(Mage::getStoreConfig('fullpagecache/cache_fullpage/learning_mode'));
        $this->setCacheBlockhtmlEnabled(Mage::helper("fullpagecache")->isCacheBlockhtmlEnabled());
    }
    ...
    public function handleBlockhtmlCache($observer)
    {
        ...
        $cacheBlockhtmlConfig = Mage::getSingleton('fullpagecache/cache_blockhtml_config');
    ...
    public function addBlockInLearningMode($observer)
    {
        ...
        $block = $observer->getEvent()->getBlock();
        Varien_Profiler::stop(Mage::helper('fullpagecache')->getBlockTimerKey($block));

        if (!($block instanceof Mage_Core_Block_Template)) {
            return;
        }

        $cacheBlockhtmlLearning = Mage::getModel('fullpagecache/cache_blockhtml_learning');
        ...
        $blockTimer = $timers[Mage::helper('fullpagecache')->getBlockTimerKey($block)];
    ...
    public function startTimer($observer)
    {
        if ($this->getData('learning_mode_enabled')) {
            ...
            Varien_Profiler::start(Mage::helper('fullpagecache')->getBlockTimerKey($observer->getEvent()->getBlock()));
        }
    }
}