...
class Magegiant_Fullpagecache_Model_Cache_Fullpage_Config extends Magegiant_Fullpagecache_Model_Cache_Config
{
    public function __construct()
    {
        $this->_init('fullpagecache/cache_fullpage_config', 'fullpagecache/cache_fullpage_config_collection');
    }
    ...
    public function tryPageMatchWithCacheFullpageConfig()
    {
        ...
        foreach ($collection as $cacheFullpageConfig) {
            if ($cacheFullpageConfig["activated"] == 1
                && $cacheFullpageConfig["full_action_name"] == Mage::app()->getFrontController()->getAction()->getFullActionName()
                && (in_array(Mage::app()->getStore()->getId(), $cacheFullpageConfig["store_id"]) || in_array(0, $cacheFullpageConfig["store_id"]))
            ) {
